<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}">
                        <span class="text-2xl font-bold text-gray-800 dark:text-gray-200">🏍 Мотосалон</span>
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('dashboard')">Личный кабинет</x-dropdown-link>
                ...
                @else
                    <div class="space-x-2">
                        <a href="{{ route('login') }}" class="text-sm text-gray-700 dark:text-gray-300 underline">Войти</a>
                        <a href="{{ route('register') }}" class="text-sm text-gray-700 dark:text-gray-300 underline">Регистрация</a>
                    </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')">Каталог</x-responsive-nav-link>
            <x-responsive-nav-link :href="route('shop.cart')">🛒 Корзина</x-responsive-nav-link>
            @auth
                @if(Auth::user()->isAdmin())
